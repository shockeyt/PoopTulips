...
function openwatch_optionsframework_options() {
    ...
    if(!empty($attributes))
    {
        ...
        $options[] = array(
            'name' => __('Position', 'openswatch'),
            'desc' => "",
            'id' => "openwatch_attribute_product_list_position",
            'std' => 1,
            'type' => "select",
            'options' => array(
                1 => __( 'Before shop loop item title', 'openswatch' ),
                2 => __( 'After shop loop item title', 'openswatch' )
            )
        );
