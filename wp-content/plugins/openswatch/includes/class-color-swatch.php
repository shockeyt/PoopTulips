...
class ColorSwatch
{
    public function init()
    {
        ...
        {
            if ( openwatch_get_option( 'openwatch_attribute_product_list_position' ) == '1' ) {
                add_action('woocommerce_before_shop_loop_item_title',array($this,'woocommerce_after_shop_loop_item'),11);
            } elseif ( openwatch_get_option( 'openwatch_attribute_product_list_position' ) == '2' ) {
                add_action('woocommerce_after_shop_loop_item_title',array($this,'woocommerce_after_shop_loop_item'),11);
            }
        }
        //add product setting
    ...
    public function woocommerce_template_loop_product_thumbnail()
    {
        ...
        if(isset($attributes[$swatch]))
        {
            ...
            if(!$tmp && $product->is_type( 'variable' ))
            {
                $variations = $product->get_available_variations();
                $tmp = array();
