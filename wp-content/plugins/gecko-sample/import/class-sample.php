...
class JAS_Gecko_Sample {
	...
	public function getDemoFilePath() {
		return JAS_GECKO_SAMPLE_PATH . '/demo-files/' . $this->filename;
	}
	...
	public function admin_menu() {
		add_submenu_page(
			'jas',
			__( 'Import Gecko Sample Data', 'gecko-sample' ),
			__( 'Install Sample Data', 'gecko-sample' ),
			'manage_options',
			'gecko-sample-data',
	...
	public function render_html() {
		if ( isset($_REQUEST['export'] ) && $_REQUEST['export'] == 1 )  {
			$this->export();
			echo "<div style='padding: 5px;'>Your export file url: <a href='" . JAS_GECKO_SAMPLE_URI . '/cache/' . $this->filename . "'>" . JAS_GECKO_SAMPLE_URI . '/cache/' . $this->filename . '</a></div>';
		} else {
			$action = isset($_REQUEST['action'] ) ? $_REQUEST['action'] : '';
			ob_start();
			$file =  JAS_GECKO_SAMPLE_PATH . '/import/view/import.php';
			require( $file );
			echo ob_get_clean();
	...
	public function import() {
		...
		if ( ! class_exists( 'WP_Import' ) ) {
			$class_wp_import = JAS_GECKO_SAMPLE_PATH . '/import/wordpress-importer.php';
		...
		$content = file_get_contents( JAS_GECKO_SAMPLE_PATH . '/demo-files/' . $view . '/' . $this->filename );
		...
		foreach ( $posts as $post ) {
			...
			$url = JAS_GECKO_SAMPLE_URI . '/demo-files/img.png';
	...
	public function export() {
		...
		$result = array();
		if ( is_writable( JAS_GECKO_SAMPLE_PATH . '/cache' ) ) {
			...
			file_put_contents(
				JAS_GECKO_SAMPLE_PATH . '/cache/' . $this->filename,
				json_encode( $result )
			);
			return true;
		}
